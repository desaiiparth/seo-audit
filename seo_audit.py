...
def enrich_with_pagespeed(
    ...
    try:
        ...
        summary["psi_error"] = f"PageSpeed preflight failed for {preflight_target}: {exc}"
        for audit in audits:
            audit.psi_note = f"pagespeed_error: {exc}"
        raise RuntimeError(summary["psi_error"]) from exc
    ...
    for idx, audit in enumerate(selected, 1):
        ...
        try:
            ...
            if inp_value is None:
                inp_value = m_audits.get("max-potential-fid", {}).get("displayValue", "unavailable")
            audit.inp = str(inp_value)
