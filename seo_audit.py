...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--oauth-client-file", default="", help="OAuth client JSON file for Google login")
...
def get_search_console_service(credentials_file: str, oauth_client_file: str = ""):
...
def enrich_with_gsc(
    ...
    auth_ready = False
    if oauth_client_file and Path(oauth_client_file).exists():
        auth_ready = True
    if credentials_file and Path(credentials_file).exists():
        auth_ready = True
    if not auth_ready or not site_url:
        summary["gsc_error"] = "GSC skipped: missing valid auth file (--oauth-client-file or --credentials-file) or --site-url."
        return summary

    try:
        service = get_search_console_service(credentials_file, oauth_client_file)
