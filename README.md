...
### 3b) (Alternative) Use OAuth login with your normal Google account
...
7. Upload `oauth_client.json` into your Codespace/repo folder.
...
OAuth login command (uses your personal Google account and stores token in `token.json`):
...
When you run OAuth for the first time:
- A browser login opens.
- Sign in with your Google account that has Search Console access.
- After approval, `token.json` is saved locally and reused on future runs.
...
- For GSC/Inspection auth, you can use either `--credentials-file` (service account) or `--oauth-client-file` (normal Google OAuth login).
